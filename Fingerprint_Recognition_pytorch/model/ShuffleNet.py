...
class ChannelShuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        N, C, H, W = x.size()
        g = self.groups
        return x.reshape(N, g, int(C / g), H, W).permute(0, 2, 1, 3, 4).contiguous().reshape(N, C, H, W)
...
class ShuffleNet(nn.Module):
    def __init__(self, planes, layers, groups, num_classes=1000):
        ...
        self.global_pool = nn.AvgPool2d(kernel_size=4, stride=1)
        self.dropout = nn.Dropout(p=0.2)
        self.linear = nn.Linear(in_features=planes[2]*1*1, out_features=num_classes)
