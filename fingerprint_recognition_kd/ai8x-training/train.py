...
tmodel = MobileNetV2(num_classes=100).cuda()
tmodel.load_state_dict(torch.load("/disks/disk2/lishengyan/mcuev/max7800-dev/ai8x-training/teacher_model.pth"))
embeddings=torch.zeros(2700,512).cuda()
lables=torch.zeros(2700).cuda()
# embeddings=torch.zeros(100,512).cuda()
# lables=torch.zeros(100).cuda()
...
def main():
    ...
    if args.evaluate:
        return evaluate_model(model, criterion, train_loader,test_loader, pylogger, activations_collectors,
    ...
    print("embeddings size:",len(embeddings),len(lables))
        ...
        if not args.earlyexit_lossweights:
            if(epoch==159):
                for i in range(len(output)):
                    #print(train_step*64+i)
                    #embeddings[target[i]] = output[i].flatten()
                    embeddings[train_step*64+i]=output[i].flatten()
                    lables[train_step*64+i]=target[i]
                #     embeddings.append(output[i].flatten().detach().cpu())
                #     lables.append(target[i])
                #     embeddings[target[i]].append(output[i].flatten().detach().cpu())
                #print(embeddings)
            #print(output.size(),target.size())
            embedding=inputs#.cuda()
            with torch.no_grad():
                for name, module in tmodel.named_children():
                    #print(name)
            ...
            if args.show_train_accuracy == 'full' or \
               (args.show_train_accuracy == 'last_batch' and train_step >= len(train_loader)-2):
                if len(output.data.shape) <= 2:
                    ...
                    #              target.flatten())
                    classerr.add(output.data.permute(0, 2, 3, 1).flatten(start_dim=0, end_dim=2),
                                 target.data.permute(0, 2, 3, 1).flatten(start_dim=0, end_dim=2))
                if not args.regression:
...
def test(test_loader, model, criterion, loggers, activations_collectors, args):
    ...
    with collectors_context(activations_collectors["test"]) as collectors:
        top1, top5, losses = _validate(test_loader, model, criterion, loggers, args,embeddings,lables)
...
def _validate(data_loader, model, criterion, loggers, args,embeddings=None,lables=None, epoch=-1, tflogger=None):
            ...
            if not args.earlyexit_thresholds:
                ...
                with torch.no_grad():
                    for name, module in tmodel.named_children():
                        ...
                        if name != 'dropout' and name != 'linear':
                            embedding = module(embedding)
                if(embeddings!=None and lables!=None):
                    # print(embeddings.size())
                    #embeddings = torch.from_numpy(np.array(embeddings))
                    #lables = torch.from_numpy(np.array(lables))
                    # embeddings_dp = embeddings
                    embeddings_dp = torch.zeros(100, 512).cuda()
                    # print("embeddings size", embeddings.size())
                    for i in range(100):
                        embeddings_dp[i] = torch.mean(embeddings[torch.where(lables == i)], 0)
                    for i in range(len(output)):
                        distance = torch.abs(output[i].flatten()- embeddings_dp)
                        distance = torch.mean(distance, dim=-1)
                        # distance = torch.pairwise_distance(output[i].view(1,-1),embeddings_dp)
                        result = torch.argmin(distance)
                        if (result == target[i]):
                            # print(result)
                            correct += 1
                    # print(embeddings[0])
                    total += len(output)
                    print("similiry result:", correct, "/", total)
...
def evaluate_model(model, criterion, train_loader,test_loader, loggers, activations_collectors, args,
                   scheduler=None):
    """
    ...
    for train_step, (inputs, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            for i in range(len(output)):
                # print(train_step*64+i)
                # embeddings[target[i]] = output[i].flatten()
                embeddings[train_step * 256 + i] = output[i].flatten()
                lables[train_step * 256 + i] = target[i]
